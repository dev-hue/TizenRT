  ...
  if (num_elements_in_axis > 0) {
    ...
    if (compute_sum) {
      ...
      for (size_t idx = 0; idx < num_outputs; ++idx) {
        ...
        float temp = float_mean * scale + bias;
        float round = static_cast<float>(static_cast<int>(temp + 0.5));
        float result = round + output_zero_point > __FLT_MAX__ ? __FLT_MAX__ : round + output_zero_point;
            /* std::min(round + output_zero_point,
                     static_cast<float>(std::numeric_limits<T>::max())); */
        result = result > __FLT_MIN__ ? result : __FLT_MIN__;
